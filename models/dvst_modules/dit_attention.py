...
class ConvInputLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for spatial_layer in self.spatial_conv_layers:
            x = spatial_layer(x)
...
class DiTNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        super().__init__()
        self.condition_input_layer = ConvInputLayer()
    ...
    def forward(
        ...
        # time
        hidden_states = torch.cat([hidden_states, self.condition_input_layer(pose_guide_conditions)], dim=1)
        timesteps = timestep
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            self.attn_adapter.to_out.apply(zero_module)
        else:
